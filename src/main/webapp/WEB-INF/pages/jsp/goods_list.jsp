...
<head>
    ...
    <script type="text/javascript">
        ...
        function joinCart_list(num, goodsId, price) {
            location.href = "<%=request.getContextPath()%>/cart/add?goodsId=" + goodsId
                + "&num=" + num + "&price=" + price;
        }
    </script>
</head>
...
<div class="container">
    ...
    <form action="<%=request.getContextPath()%>/goods/listByPageCate" method="get" id="form1">
                        ...
                        <c:forEach items="${goodses }" var="goods" varStatus="s">
                            ...
                            <li><span>${s.index+1}</span>
                                <a title="${goods.name}" class="p_name"
                                   href="<%=request.getContextPath()%>/goods/view?goodsId=${goods.id}">${goods.name}</a></li>
                        </c:forEach>
                    </ul>
                </div>
            </div>
            ...
            <div class="display_title">
                ...
                <ul class="tag">
                    <li id="sellnum" class="current"><span class="l"></span><span class="r"></span>
                        <a href="<%=request.getContextPath()%>/goods/listByPageCate?categoryId=${category.id}&order=sellnum&index=1">销量<span
                                class="">&nbsp;</span></a>
                        <input id="index" name="index" type="hidden" value="${index}"/></li>
                    <li id="role" class="" onclick=""><span class="l"></span><span class="r"></span>
                        <a href="<%=request.getContextPath()%>/goods/listByPageCate?categoryId=${category.id}&order=role&index=2">热度<span class="">&nbsp;</span></a>
                    </li>
                    <li id="price2" class=""><span class="l"></span><span class="r"></span>
                        <a href="<%=request.getContextPath()%>/goods/listByPageCate?categoryId=${category.id}&order=price2&index=3">价格<span
                                class="">&nbsp;</span></a></li>
                    <li id="selltime" class=""><span class="l"></span><span class="r"></span>
                        <a href="<%=request.getContextPath()%>/goods/listByPageCate?categoryId=${category.id}&order=selltime&index=4">最新上架<span
                                class="">&nbsp;</span></a></li>
                </ul>
            </div>
            ...
            <ul class="display_list clearfix m_10">
                ...
                <c:forEach items="${pageBean.data }" var="goods">
                    <li class="clearfix win">
                        ...
                        <div class="pic">
                            <a title="${goods.name }"
                               href="<%=request.getContextPath()%>/goods/view?goodsId=${goods.id }">
                                <img src="${goods.thumbnail }" width="200" height="200" alt="${goods.name }"
                                     title="${goods.name }"/></a>
                        </div>
                        <h3 class="title">
                            <a title="${goods.name }" class="p_name"
                               href="<%=request.getContextPath()%>/goods/view?goodsId=${goods.id}">${goods.name }</a>
                                <%--<span>总销量：${goods.sellNum }<a class="blue" href="">( 0人评论 )</a></span>--%>
                            <span>总销量：${goods.sellNum }<a class="blue"
                                                          href="<%=request.getContextPath()%>/goods/view?goodsId=${goods.id}">评论</a></span>
                            <span class='grade'><i style='width: 0px'></i></span>
                        </h3>
                        <div class="handle">
                            <label class="btn_gray_m"><img
                                    src="<%=request.getContextPath()%>/images/front/ucenter/shopping.gif" width="15" height="15"/>
                                <input type="button" value="加入购物车"
                                       onclick="joinCart_list('1','${goods.id}','${goods.price2}');"></label>
